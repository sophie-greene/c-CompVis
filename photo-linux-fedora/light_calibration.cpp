...
/*******************************************************************************
Calculates light directions. Lights directions are approximated as the std::vector going 
...
Input:
	...
	cameras_height: std::vector with the height of each camera as seen along the Z axis of the camera itself
	im_w: image width
	im_h: image height
...
cv::Mat calc_light_direction(std::vector<cam_params_t> params,int cam_num,std::vector<int> available_lights,std::vector<double> &cameras_height, int im_w, int im_h) {

	std::vector<std::vector<cv::Mat> >  light_dirs;
	...
	//normal std::vector to the floor plane
	cv::Mat n(3,1,CV_64F);
	n.at<double>(0) = 0;
	n.at<double>(1) = 0;
	n.at<double>(2) = 1;
	
	//std::vector going along the Z axis of the camera
	...
	//change std::vector from camera to world coordinates
	aux = M.inv()*aux;
	...
	*/
	...
	l_dir = l_dir / calc_norm(l_dir,3);				//this is a normal std::vector in the direction of the Z axis of the camera but seen in world coordinates

...
std::vector<std::vector<cv::Point> >  read_intrinsic_parameters(std::string file_name) {
	...
	int max_x = 0;
	int max_y = 0;
	while(read>> line>> line>> line>> a>> 
		val[0]>> a>> val[1]>> a>> val[2]>> a>> val[3]>> a>> 
		val[4]>> a>> val[5]>> a>> val[6]>> a>> val[7]>> a>> 
		val[8]>> a>> val[9]>> a>> val[10]>> a>> val[11]>> a>> 
		val[12]>> a>> val[13]>> a>> val[14]>> a>> val[15]>> a) {
		std::vector<cv::Point> aux_v;
		cv::Point aux_p;
...
void build_ground_truth_cloud(pcl::visualization::PCLVisualizer &viewer, std::vector<cam_params_t> params, std::vector<std::vector<cv::Point> >  laser_pixels) {
	...
	/*std::vector<IplImage*> images(8);
	std::vector<IplImage*> small_images(8);
	for (int i = 0; i < 8; i++)
	...
	*/
	...
	std::vector<int> cam_counter(8,0);
	...
	for (int i = FRAME_OFFSET; i < laser_pixels.size(); i++) {
		//std::cout << i << "/" << laser_pixels.size()<< std::endl;
		...
		*/
		for (int j = 0; j < 8; j++) {
			...
			for (int w = -FRAME_OFFSET; w <= FRAME_OFFSET; w++) {
				...
				for (int k = 0; k < 8; k++) {
					...
					//std::cout << j << " / "<<x1 << " "<<y1<<" "<<k<<" / "<<x2<<" "<<y2<<std::endl;
					...
					continue;
						...
						*/
						...
						if (pcl::lineWithLineIntersection(line1_coeff,line3_coeff,intersection_point2,INTERSECTION_THRESH)) {
							float point_dist = std::sqrt(double(
		...
		/*	if (j == min_k) {
			...
			if ((min_dist < 0.05) && (min_dist2 < 0.05)) {
				std::cout << j << " [" << x1<<" "<<y1<<"] "<<min_k<<" [" << min_x2<<" "<<min_y2<<"] "<<"dist " << min_dist << " " << min_dist2 << std::endl;
	...
	for (int i = 0; i < 8; i++)
		std::cout <<cam_counter[i] << std::endl;
...
void test(std::vector<std::vector<cv::Point> >  laser_pixels) {
	...
	std::vector<std::vector<cv::Point> >  pixels(8);
	for (int i = 0; i < 8; i++)
		small_images[i] = cvCreateImage(cvSize(176,144),IPL_DEPTH_8U,3);	
	images[0] = cvLoadImage("cam0.bmp");
	images[1] = cvLoadImage("cam1.bmp");
	...
	for (int i = 5; i < laser_pixels.size(); i++) {
		...
		cvResize(images[6],small_images[6]);
		cvResize(images[7],small_images[7]);
		//std::cout << laser_pixels[i] << std::endl;

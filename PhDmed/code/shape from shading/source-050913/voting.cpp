...
/*
void acc_votes(std::vector<std::vector<std::vector<std::vector<matchings_t> > > >  &matches3D,pcl::visualization::PCLVisualizer &viewer, int h, int w) {


	std::vector<std::vector<std::vector<std::vector<vote_t> > > >  votes3D(CAM_NUM,std::vector<std::vector<std::vector<vote_t> > >(h,std::vector<std::vector<vote_t> > (w)));
	
	
	
	std::vector<vote_to_pixel_t> pixel_votes;
	vote_to_pixel_t aux;
	...
	for (int cam_idx = 0; cam_idx < CAM_NUM; cam_idx++) {
		for (int i = 0; i < height; i++) {
			for (int j = 0; j < width; j++) {
				std::vector <vote_to_pixel_t> related_pixels; 
			...
			for (int j = 0; j < width; j++) {
				for (int k = 0; k < matches3D[cam_idx][i][j].size(); k++) {
					...
					if (aux.paired_cam == cam_idx) {
						std::cout << "ERROR: " << cam_idx << " " << aux.paired_cam << std::endl;
						exit(0);
					}
	...
	for (int i = 0; i < sampled_cloud->points.size(); i++) {
		...
		if (v.vote > VOTE_THRESH) {
			...
			for (int j = 0; j < nIdx.size(); j++) {
				...
				if (cam == v.paired_cam) {
						std::cout << "ERROR: " << cam << " " << v.paired_cam << std::endl;
						exit(0);
					}
			...
			for (int j = 0; j < width; j++) {
				...
				if (max > -1) {
					...
					if (cam_idx == matches3D[cam_idx][i][j][0].paired_cam) {
							std::cout << "ERROR: " << cam_idx << " " << matches3D[cam_idx][i][j][0].paired_cam << std::endl;
							exit(0);
						}

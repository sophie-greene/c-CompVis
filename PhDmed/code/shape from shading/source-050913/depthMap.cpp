...
cv::Mat calc_depthMap(std::vector<std::vector<cv::Mat> >  normals, IplImage* mask,Engine *ep) {
	...
	std::vector<double> objectPixelRow;
	std::vector<double> objectPixelCol;
	...
	std::vector<std::vector<int> >  index = std::vector<std::vector<int> > (nrows,std::vector<int>(ncols,-1));
	...
	std::vector<double> colIndex;

	std::vector<double> rowIndexF;
	std::vector<double> colIndexF;

	std::vector<double> sparseMat_valF;
	std::vector<double> sparseMat_val;

	std::vector<double> v(numPixels*2);
	std::vector<double> g(numPixels*2,100);

	std::vector<double> image_dims(2);
	...
	//std::cout << numPixels*2 << "Triplet size: " << tripletList.size() << std::endl;
	...
	//std::cout << "non zeros in M: " << M.nonZeros() << std::endl;
...
cv::Mat calc_depthMap(double depth_factor, std::vector<std::vector<cv::Mat> >  normals, IplImage* mask,Engine *ep,std::vector<std::vector<std::vector<matchings_t> > > points3D, cam_params_t params, double cam_h,pcl::visualization::PCLVisualizer &viewer, double weight) {
	...
	*/
	...
	std::vector<double> v(numPixels*2,0);
	std::vector<double> g(numPixels*2,0);

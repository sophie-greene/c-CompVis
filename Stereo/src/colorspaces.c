...
/*
 * added by Oni to convert to RGB which is more useful :P
 */
 
unsigned short int clamp (unsigned int value){
	unsigned short int rval = value > 255 ? 255: value;
	rval = rval < 0 ? 0 : rval;
	return rval;
}
 
void yuvtorgb(unsigned short int y, unsigned short int u, unsigned short int v, unsigned short int *r, unsigned short int *g, unsigned short int *b){
	/* *r = y + v * ((1 - 0.299)/0.615);
	*g = y - u * ((0.114*(1-0.114))/(0.436*0.587)) - v *((0.299*(1 - 0.299))/(0.615*0.587));
	*b = y + v * ((1 - 0.114)/0.436);*/
	
	*r = clamp(y + 1.402 * (v -128));
	*g = clamp(y - 0.344 * (u -128) -0.714 * (v - 128));
	*b = clamp(y + 1.772 * (v -128));

}  



void yuv422ptoRGB(int * out, unsigned char *pic, int width) {
	int j, k;
	unsigned char *pic0, *pic1;
	int *outy, *outu, *outv;
	int outy1 = 0;
	...
	int outu1 = 0;
	int outv1 = 0;
	unsigned short int y,u,v,r,g,b;
	...
	for (j = 0; j < 4; j++) 
	{
		for (k = 0; k < 8; k++) 
		{
			...
			// Line 1
			y = CLIP(outy[outy1]); 
			u = CLIP(128 + outu[outu1]);
			v = CLIP(128 + outv[outv1]);
			...
			*pic0++ = b;
			...
			y = CLIP(outy[outy2]);
			u = CLIP(128 + outu[outu1+8]);
			v = CLIP(128 + outv[outv1+8]);
			yuvtorgb(y,u,v,&r,&g,&b);

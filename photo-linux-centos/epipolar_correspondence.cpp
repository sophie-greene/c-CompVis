...
std::vector<std::vector<cv::Point> >  find_epipolar_lines(cv::Mat E, int im_point_x, int im_point_y, cv::Mat Al, cv::Mat Ar, IplImage *im1, IplImage *im2,IplImage *mask1, IplImage *mask2, int line_num) {
	...
	std::vector<cv::Point> intersections = find_mask_intersections(a,b,c,mask2);
	...
	float line_length = std::sqrt(double((im_p1.x - im_p2.x)*(im_p1.x - im_p2.x) + (im_p1.y - im_p2.y)*(im_p1.y - im_p2.y)));
	...
	/*
	std::vector<Point> segment_on_mask;
	...
	*/
	...
	line_length = std::sqrt(double((im_p1.x - im_p2.x)*(im_p1.x - im_p2.x) + (im_p1.y - im_p2.y)*(im_p1.y - im_p2.y)));

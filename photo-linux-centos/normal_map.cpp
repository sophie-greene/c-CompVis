...
void weight_intensisties(cv::Mat intensities,cv::Mat &intensities_weighted, cv::Mat l, cv::Mat &l_weighted, int &valid, float low_t, float high_t, float weight_val) {
	...
	for (int i = 0; i < intensities.rows; i++) {
		...
		if (intensities.at<double>(i) < low_t || intensities.at<double>(i) > high_t) {
			//std::cout <<intensities.at<double>(i) << std::endl;
			weight = weight_val;
		}
		else {
			++valid;
...
void compute_images_max(std::vector<IplImage*> images, std::vector<float> &maxs) {
...
Input: 
	images: std::vector containing the images corresponding to different light sources
	mask_iamge: we do not compute normals for pixels outside the mask
	l: light directions
	m: compound rotation and translation matrix
	trans. translation std::vector for the cam
Output:
	normal map, each normal is a cv::Mat containing x,y,z coordinates of the normal
***********************************************************************************************/


std::vector<std::vector<cv::Mat> >  calc_normal_map(std::vector<IplImage*> images, IplImage* mask_image, cv::Mat l, cv::Mat m, cv::Mat trans, float low_t, float high_t, float weight) {
	
	
	//std::cout << images[1]->height << " " << images[1]->width << std::endl;
	std::vector<std::vector<cv::Mat> >  normals(images[1]->height, std::vector<cv::Mat>(images[1]->width));
		...
		for (int j = 0; j < images[1]->width; j++) {
			...
			if (CV_IMAGE_ELEM(mask_image, uchar, i,j) > 100 ) {		//if we are inside the mask
				...
				for (int k = 0; k < images.size(); k++) {	
					//std::cout << "image: " << (float)CV_IMAGE_ELEM(images[k],uchar,i,j)/255.0 << std::endl;
					...
					//std::cout << "image: " << intensities_data[k] << std::endl;
				}

				//use the normal of the intensities std::vector to determine is the pixel is feasible to
				...
				for (int k = 0; k < intensities.rows; k++) {
					...
					//std::cout << maxs[k] << std::endl;
				}
				avg /= (float)intensities.rows;

				//std::cout << avg << " " << low_t << " " << high_t  << std::endl;

...
int main() {
	...
	*/
	...
	std::vector<std::vector<cv::Point> > pixel_matches;
        std::vector<std::vector<vector<vector<matchings_t> > > > matches3D(CAM_NUM,vector<vector<vector<matchings_t> > >(im_height,vector<vector<matchings_t> >(im_width)));
	...
	std::vector<std::vector<std::vector<PointXYZINormal> > > surfaces(CAM_NUM);

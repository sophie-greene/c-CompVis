...
*/
void acc_votes(std::vector<std::vector<std::vector<std::vector<matchings_t> > > >& matches3D,
               pcl::visualization::PCLVisualizer &viewer, int h, int w ,std::vector<IplImage*> imgs) {
    std::vector<std::vector<std::vector<std::vector<vote_t> > > > votes3D
            (CAM_NUM,vector<vector<vector<vote_t> > >(h,std::vector<std::vector<vote_t> >(w)));
    ...
    for (int i = 0; i < sampled_cloud->points.size(); i++) {
        kdtree.radiusSearch(sampled_cloud->points[i],VOTE_R,nIdx,nDist);
        v.vote = nIdx.size();

...
public:
  ...
  Camera(CameraType type, cv::Mat K = cv::Mat(), cv::Mat dist = cv::Mat(), double epsilon = -1) : type_(type), K_(K), dist_(dist), epsilon_(epsilon) {}
  cv::Mat getParam() {return K_;}
  cv::Mat getDist() {return dist_;}
  double getEpsilon() {return epsilon_;}
  CameraType getType() {return type_;}

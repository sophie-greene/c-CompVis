...
void add_coloured_meshes(vector <IplImage*> masks,vector <IplImage*> ims, vector<vector<vector<PointXYZINormal>>> surfaces, vector<cam_params_t>& params,pcl::visualization::PCLVisualizer &viewer, float final_smooth_r,pcl::PointCloud<pcl::PointXYZ>::Ptr hull_cloud,pcl::PolygonMesh hull_triangles, float leaf_size,float intersections_search_r) {

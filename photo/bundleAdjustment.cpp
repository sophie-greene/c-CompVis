...
void create_avg_surface(vector<pcl::PointCloud<PointXYZINormal>::Ptr> clouds,pcl::visualization::PCLVisualizer &viewer, pcl::PolygonMesh &triangles,pcl::PointCloud<pcl::PointXYZ>::Ptr &mls_points, float voxel_size, float mls_radius) {
...
vector<vector<vector<vector<matchings_t>>>> refine_depth_constraints_one_cam(int cam_idx, vector<vector<vector<cv::Mat>>> &normal_maps,vector<vector<vector<PointXYZINormal>>> surfaces, vector<cam_params_t>& params,pcl::visualization::PCLVisualizer &viewer) {
		...
		for (int j = 0; j < sampled_all_clouds->points.size();j++) { 
				...
				//matches3D[i][clouds_pixel_idx[i][j].y][clouds_pixel_idx[i][j].x].push_back(aux);
					...
					*/
					...
					//float dist2 = sqrt(double((p3.x-p2.x)*(p3.x-p2.x)+(p3.y-p2.y)*(p3.y-p2.y)+(p3.z-p2.z)*(p3.z-p2.z)));	
					float dist1 = sqrt(double((p1.x-p3.x)*(p1.x-p3.x)+(p1.y-p3.y)*(p1.y-p3.y)+(p1.z-p3.z)*(p1.z-p3.z)));
...
void smoothing(vector<pcl::PointCloud<pcl::PointXYZINormal>::Ptr> &clouds, vector<cam_params_t>& params,pcl::visualization::PCLVisualizer &viewer) {
	
	vector<pcl::PointCloud<pcl::PointXYZ>::Ptr> deformed_clouds(CAM_NUM);
	pcl::PointCloud<pcl::PointXYZ>::Ptr all_clouds(new pcl::PointCloud<PointXYZ>);
	
	
	
	for (int i = 0; i < CAM_NUM;i++) {
		deformed_clouds[i] = pcl::PointCloud<pcl::PointXYZ>::Ptr(new pcl::PointCloud<PointXYZ>);
		for (int j = 0; j < clouds[i]->points.size(); j++) {
			PointXYZ p;
			p.x = clouds[i]->points[j].x;
			p.y = clouds[i]->points[j].y;
	...
	viewer.addPointCloud<PointNormal>(mls_points,"moving_least_squares");


}

void deform_clouds(vector<pcl::PointCloud<pcl::PointXYZINormal>::Ptr> &clouds, vector<cam_params_t>& params,pcl::visualization::PCLVisualizer &viewer) {


	
	
	vector<pcl::PointCloud<pcl::PointXYZINormal>::Ptr> deformed_clouds(CAM_NUM);
	pcl::PointCloud<pcl::PointXYZINormal>::Ptr all_clouds(new pcl::PointCloud<PointXYZINormal>);
	pcl::PointCloud<pcl::PointXYZINormal>::Ptr sampled_all_clouds(new pcl::PointCloud<PointXYZINormal>);
	...
	for (int i = 0; i < CAM_NUM; i++) {
		...
		normals[i] = pcl::PointCloud<pcl::Normal>::Ptr(new pcl::PointCloud<Normal>);
		ne.setInputCloud (clouds[i]);
		ne.setKSearch(20);
		ne.compute (*normals[i]);
		for (int j = 0; j < normals[i]->points.size(); j++) {
	...
	for (int i = 0; i < CAM_NUM;i++) {
		deformed_clouds[i] = pcl::PointCloud<pcl::PointXYZINormal>::Ptr(new pcl::PointCloud<PointXYZINormal>);
		*all_clouds = *all_clouds + *clouds[i];
	}
	pcl::VoxelGrid<pcl::PointXYZINormal> sor;
	...
	*/
	...
	//	viewer.removeShape("sphere");
		...
		*/
		stringstream ss;
		ss<<i;
		viewer.addPointCloud<PointXYZINormal>(deformed_clouds[i],"deformed_cloud"+ss.str());
